...
function changeVariant(p, update, trigger) {
    ...
    if (ch) {
        ...
        loadFiles(p, set);
        if(p.copyOf) p.vars[fn] = p.rawChannels;
    }
}
...
function removePhone(p) {
    console.log("removePhone triggered");
...
function iso226(phon, targetFreq) {
    ...
    for(let i = 0; i < f.length; i++) {
        Lp.push(10 / a_f[i] * Math.log10(Math.pow(4*Math.pow(10, -10), 0.3 - a_f[i]) * (Math.pow(10, 0.03 * Ln) - Math.pow(10, 0.072)) + Math.pow(10, a_f[i] * (T_f[i] + L_U[i]) / 10)) - L_U[i]);
    }
...
function change_multiplier(val) {
    ...
    for(let i=0;i<activePhones.length;i++) {
        if(activePhones[i].loudness && activePhones[i].loudness[activePhones[i].fileName])
            temp.push(activePhones[i].loudness[activePhones[i].fileName]);
        else temp.push(85);
        if(activePhones[i].isTarget) continue;
